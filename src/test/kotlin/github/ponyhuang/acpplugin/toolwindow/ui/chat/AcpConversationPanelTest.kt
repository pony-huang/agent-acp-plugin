...
import github.ponyhuang.acpplugin.toolwindow.ui.toolcall.*
...
class AcpConversationPanelTest : BasePlatformTestCase() {
    ...
    private val noOpPermissionBeforeSubmit: () -> Unit = { }
    private val noOpPermissionCardUpdated: () -> Unit = { }
    ...
    fun testMessageCardsAndPermissionCardsFillConversationWidth() {
        ...
        try {
            ...
            val messageCard = instantiatePrivatePanel(
                "github.ponyhuang.acpplugin.toolwindow.ui.chat.MessageCardPanel",
                arrayOf(
                    Project::class.java,
                    ...
                    Function2::class.java,
                    Function0::class.java,
                    Class.forName("github.ponyhuang.acpplugin.toolwindow.ui.chat.MessagePromptState"),
                ...
                arrayOf<Any?>(
                    ...
                    noOpPermissionBeforeSubmit,
                    noOpPermissionCardUpdated,
    ...
    fun testThoughtPanelLimitsExpandedHeightWithInternalScroll() {
        ...
        val thoughtPanel = instantiatePrivatePanel(
            "github.ponyhuang.acpplugin.toolwindow.ui.chat.ThoughtPanel",
    ...
    fun testConversationRenderKeepsViewportPinnedAtBottomDuringStreamingUpdates() {
        val disposable = Disposer.newDisposable()
        val panel = ChatViewPanel(project, disposable)
        try {
            val host = JPanel(BorderLayout()).apply {
            ...
            val initialMessages = overflowingConversation(messageCount = 8)
            renderConversation(panel, initialMessages)
            layoutRecursively(host)

            val scrollBar = conversationScrollBar(panel)
            scrollBar.value = scrollBar.maximum - scrollBar.model.extent
            PlatformTestUtil.dispatchAllEventsInIdeEventQueue()

            val updatedLast = initialMessages.last().copy(
                content = initialMessages.last().content + "\n\n" + List(10) { "Streaming line ${it + 1} that should keep the viewport pinned to the bottom." }.joinToString("\n")
            )
            renderConversation(panel, initialMessages.dropLast(1) + updatedLast)
            layoutRecursively(host)
            PlatformTestUtil.dispatchAllEventsInIdeEventQueue()

            assertTrue(
                "value=${scrollBar.value} extent=${scrollBar.model.extent} max=${scrollBar.maximum}",
                scrollBar.value + scrollBar.model.extent >= scrollBar.maximum - JBUI.scale(4)
            )
        } finally {
            Disposer.dispose(disposable)
        }
    ...
    fun testConversationRenderPreservesManualScrollOffsetWhenUserScrolledUp() {
        val disposable = Disposer.newDisposable()
        val panel = ChatViewPanel(project, disposable)
        try {
            val host = JPanel(BorderLayout()).apply {
            ...
            val updatedLast = initialMessages.last().copy(
                content = initialMessages.last().content + "\n\n" + List(12) { "Additional rendered line ${it + 1} should not pull the user back down." }.joinToString("\n")
            )
            renderConversation(panel, initialMessages.dropLast(1) + updatedLast)
            layoutRecursively(host)
            PlatformTestUtil.dispatchAllEventsInIdeEventQueue()
    ...
    fun testThoughtToggleKeepsViewportPinnedAtBottomAfterRelayout() {
        val disposable = Disposer.newDisposable()
        val panel = ChatViewPanel(project, disposable)
        try {
            val host = JPanel(BorderLayout()).apply {
            ...
            val fillerMessages = overflowingConversation(messageCount = 6)
            val thoughtMessage = AcpSessionService.ChatMessage(
                id = "assistant-thought-bottom-scroll",
                role = "assistant",
                content = "Final answer",
                entries = listOf(
                    AcpSessionService.MessageEntry.Thought(
                        List(20) { "Thought line ${it + 1} should expand the final row and trigger a deferred relayout." }.joinToString("\n\n")
                    ),
                    AcpSessionService.MessageEntry.Content("Final answer")
                )
            )
            renderConversation(panel, fillerMessages + thoughtMessage)
            layoutRecursively(host)
            ...
            val row = messageRowComponent(panel, "assistant-thought-bottom-scroll")
            val thoughtPanel = findByClassName(row, "ThoughtPanel") as javax.swing.JComponent
            val toggle = findAllByType(thoughtPanel, ActionLink::class.java).first { it.text == "Show Thinking" }
    ...
    fun testPermissionCardSubmitKeepsViewportPinnedAtBottom() {
        val disposable = Disposer.newDisposable()
        val panel = ChatViewPanel(project, disposable)
        try {
            val host = JPanel(BorderLayout()).apply {
            ...
            val initialRequest = AcpSessionService.PermissionRequestInfo(
                requestId = "request-submit-bottom",
                toolCallId = "tool-submit-bottom",
                title = "Allow command",
                options = listOf(
                    AcpSessionService.PermissionOptionInfo(
                        optionId = "allow-once",
            ...
            renderConversation(panel, fillerMessages + permissionMessage("assistant-permission-submit-bottom", initialRequest))
            layoutRecursively(host)
            ...
            val card = permissionCard(panel, "request-submit-bottom") as JPanel
            card.javaClass.getDeclaredMethod(
                "updateRequest",
                AcpSessionService.PermissionRequestInfo::class.java
            ).apply {
                isAccessible = true
            }.invoke(card, updatedRequest)
            PlatformTestUtil.dispatchAllEventsInIdeEventQueue()
            layoutRecursively(host)
            PlatformTestUtil.dispatchAllEventsInIdeEventQueue()
    ...
    fun testPermissionCardSubmitFollowedByStreamingOutputKeepsViewportPinnedAtBottom() {
        val disposable = Disposer.newDisposable()
        val panel = ChatViewPanel(project, disposable)
        try {
            val host = JPanel(BorderLayout()).apply {
            ...
            val fillerMessages = overflowingConversation(messageCount = 6)
            val initialRequest = AcpSessionService.PermissionRequestInfo(
                requestId = "request-submit-stream-bottom",
                toolCallId = "tool-submit-stream-bottom",
                title = "Allow command",
                options = listOf(
                    AcpSessionService.PermissionOptionInfo(
                        optionId = "allow-once",
            ...
            val initialPermissionMessage = permissionMessage(
                "assistant-permission-submit-stream-bottom",
                initialRequest
            )
            renderConversation(panel, fillerMessages + initialPermissionMessage)
            layoutRecursively(host)
            ...
            val updatedPermissionMessage = AcpSessionService.ChatMessage(
                id = "assistant-permission-submit-stream-bottom",
                role = "assistant",
                content = "Permission granted. Continuing with the task.\n\n" +
                    List(10) { "Streaming follow-up line ${it + 1} should not push the main viewport upward before settling." }
                        .joinToString("\n"),
                entries = listOf(
                    AcpSessionService.MessageEntry.PermissionRequest(updatedRequest),
                    AcpSessionService.MessageEntry.Content(
                        "Permission granted. Continuing with the task.\n\n" +
                            List(10) { "Streaming follow-up line ${it + 1} should not push the main viewport upward before settling." }
                                .joinToString("\n")
                    )
                )
            ...
            renderConversation(panel, fillerMessages + updatedPermissionMessage)
            layoutRecursively(host)
            PlatformTestUtil.dispatchAllEventsInIdeEventQueue()
    ...
    fun testExpandedThoughtPanelRewrapsWhenHostWidthShrinks() {
        val disposable = Disposer.newDisposable()
        val panel = ChatViewPanel(project, disposable)
        try {
            val widthSensitiveThought = buildString {
                repeat(14) { index ->
                    appendLine("Thinking paragraph ${index + 1} should stay fully visible after the tool window becomes narrower and forces a taller wrapped layout.")
                    appendLine()
                }
            }.trim()
            val host = JPanel(BorderLayout()).apply {
            ...
            val row = messageRowComponent(panel, "assistant-thinking-shrink")
            val thoughtPanel = findByClassName(row, "ThoughtPanel") as javax.swing.JComponent
            val scrollPane = thoughtPanel.javaClass.getDeclaredField("scrollPane").apply {
                isAccessible = true
            }.get(thoughtPanel) as JBScrollPane
            ...
            val wideViewportWidth = scrollPane.viewport.width
            val widePreferredHeight = markdownPane.preferredSize.height

            host.setSize(280, 560)
            host.doLayout()
            PlatformTestUtil.dispatchAllEventsInIdeEventQueue()
            PlatformTestUtil.dispatchAllEventsInIdeEventQueue()
            layoutRecursively(host)

            assertTrue(scrollPane.viewport.width < wideViewportWidth)
            assertTrue(
                "widePreferredHeight=$widePreferredHeight shrunkPreferredHeight=${markdownPane.preferredSize.height}",
                markdownPane.preferredSize.height >= widePreferredHeight
            )
            assertTrue(
                "markdown=${markdownPane.bounds} viewport=${scrollPane.viewport.bounds} viewSize=${scrollPane.viewport.viewSize}",
                scrollPane.viewport.viewSize.height >= markdownPane.preferredSize.height.coerceAtMost(JBUI.scale(240))
            )
            assertTrue(
                "markdown=${markdownPane.bounds} viewport=${scrollPane.viewport.bounds}",
                markdownPane.width <= scrollPane.viewport.width
    ...
    fun testToolCallRowUsesAnimatedRunningStatusIcon() {
        ...
        val statusIcon = findByClassName(row, "ToolStatusIcon") as JBLabel
        val timerField = statusIcon.javaClass.superclass.getDeclaredField("timer").apply {
            isAccessible = true
        }
        ...
        assertNotNull(timerField.get(statusIcon))
    }
    ...
    fun testToolCallPresentationMapperBuildsNavigationTitleForReadableFile() {
        myFixture.addFileToProject("src/Main.kt", "class Main")
    ...
    fun testToolCallPresentationMapperPrefersFailureDetailsOverContentSummary() {
        val model = ToolCallPresentationMapper(project).map(
            AcpSessionService.ToolCallInfo(
...
- Location: `openspec/changes/optimize-code-structure/`
    ...
    private fun instantiateToolCallRow(toolCall: AcpSessionService.ToolCallInfo): javax.swing.JComponent {
        return instantiatePrivatePanel(
            "github.ponyhuang.acpplugin.toolwindow.ui.toolcall.ToolCallRow",
    ...
    private fun messagePromptState(name: String): Any {
        val promptStateClass = Class.forName("github.ponyhuang.acpplugin.toolwindow.ui.chat.MessagePromptState")
    ...
    private fun conversationScrollBar(panel: ChatViewPanel) =
        (panel.javaClass.getDeclaredField("messageScrollPane").apply {
            isAccessible = true
        }.get(panel) as JBScrollPane).verticalScrollBar

    private fun overflowingConversation(messageCount: Int): List<AcpSessionService.ChatMessage> {
        return (1..messageCount).map { index ->
            AcpSessionService.ChatMessage(
                id = "message-$index",
                role = if (index % 2 == 0) "assistant" else "user",
                content = List(4) {
                    "Message $index line ${it + 1} is deliberately long so the conversation grows beyond the viewport height and wraps across the available width."
                }.joinToString("\n")
            )
        }
    }
