...
import github.ponyhuang.acpplugin.toolwindow.ui.ConversationRenderStateMapper
import github.ponyhuang.acpplugin.toolwindow.ui.ConversationViewState
import github.ponyhuang.acpplugin.toolwindow.ui.hasRenderableContent
import github.ponyhuang.acpplugin.toolwindow.ui.MessageRenderModel
import github.ponyhuang.acpplugin.toolwindow.ui.MessageRowChildMutation
import github.ponyhuang.acpplugin.toolwindow.ui.MessageRowReconciliationPolicy
import github.ponyhuang.acpplugin.toolwindow.ui.PermissionRequestCardPanel
import github.ponyhuang.acpplugin.toolwindow.ui.ScrollSnapshot
import github.ponyhuang.acpplugin.toolwindow.ui.SmartScroller
...
class ChatViewPanel(
    ...
    private val renderStateMapper = ConversationRenderStateMapper()
    private val rowReconciliationPolicy = MessageRowReconciliationPolicy()
    ...
    private var pendingScrollRestore: ScrollRestoreRequest? = null
    ...
    private fun render(state: ConversationViewState) {
        ...
        ApplicationManager.getApplication().invokeLater {
            ...
            val scrollRestoreRequest = captureScrollRestoreRequest()
            val renderState = renderStateMapper.map(state, expandedThoughts)
            logger.info(
                "[ChatRender] render requested: totalMessages=${renderState.allMessages.size}, " +
                    "visibleMessages=${renderState.visibleMessages.size}, isLoading=${state.isLoading}, " +
                    "lastStopReason=${state.lastStopReason}, mode=${renderState.renderMode.name.lowercase()}"
            )
            renderState.latestAssistantMessage?.let { latestAssistant ->
                logger.info(
            ...
            val reconciled = runCatching {
                reconcileRows(renderState.renderModels, renderState.showEmptyState)
            }.getOrElse {
                renderFully(renderState.renderModels, renderState.showEmptyState)
                true
            }
            ...
            if (reconciled) {
                ...
                scheduleScrollRestore(scrollRestoreRequest, requestedVersion)
    ...
    private fun reconcileRows(renderModels: List<MessageRenderModel>, showEmptyState: Boolean): Boolean {
        ...
        val planBeforeUpdates = rowReconciliationPolicy.decide(
            previousMessageIds = previousIds,
            nextMessageIds = nextIds,
            showEmptyState = showEmptyState,
            hadEmptyState = hasEmptyState,
            replacedExistingRow = false
        )
        planBeforeUpdates.staleMessageIds.forEach { staleId ->
        ...
        val reconciliationPlan = rowReconciliationPolicy.decide(
            previousMessageIds = previousIds,
            nextMessageIds = nextIds,
            showEmptyState = showEmptyState,
            hadEmptyState = hasEmptyState,
            replacedExistingRow = replacedExistingRow
        )

        when (val mutation = reconciliationPlan.mutation) {
            MessageRowChildMutation.Keep -> Unit
            is MessageRowChildMutation.Append -> {
                appendRows(mutation.appendedMessageIds, startRow = mutation.startRow)
                ensureSpacerAtRow(nextIds.size)
            }
            is MessageRowChildMutation.Rebuild -> rebuildPanelChildren(
                orderedIds = mutation.orderedMessageIds,
                showEmptyState = mutation.showEmptyState
            )
        }
    ...
    private fun createRowController(model: MessageRenderModel): MessageRowController {
        return MessageRowController(
            ...
            onPermissionBeforeSubmit = {
                preserveScrollForUpcomingAsyncMutation()
            },
            onPermissionCardUpdated = {
    ...
    private fun refreshPermissionCards(requests: List<AcpSessionService.PermissionRequestInfo>) {
        ApplicationManager.getApplication().invokeLater {
            val restoreRequest = pendingScrollRestore ?: captureScrollRestoreRequest()
            pendingScrollRestore = restoreRequest
    ...
    private fun captureScrollRestoreRequest(): ScrollRestoreRequest {
        val model = messageScrollPane.verticalScrollBar.model
        return ScrollRestoreRequest(
            shouldPinToEnd = smartScroller.isPinnedToEnd(),
    ...
    private fun scheduleScrollRestore(request: ScrollRestoreRequest, requestedVersion: Int) {
        pendingScrollRestore = request
        SwingUtilities.invokeLater {
            if (requestedVersion != renderVersion.get()) {
                return@invokeLater
            }

            restoreScrollPosition(request)
            ...
            SwingUtilities.invokeLater {
                ...
                messagePanel.repaint()
                restoreScrollPosition(request)
                if (pendingScrollRestore === request) {
    ...
    private fun scheduleDeferredScrollRestore() {
        val request = pendingScrollRestore ?: captureScrollRestoreRequest()
        pendingScrollRestore = request
        SwingUtilities.invokeLater {
            if (pendingScrollRestore !== request) {
                return@invokeLater
    ...
    private fun preserveScrollForUpcomingAsyncMutation() {
        pendingScrollRestore = captureScrollRestoreRequest()
    }

    private fun restoreScrollPosition(request: ScrollRestoreRequest) {
        if (request.shouldPinToEnd) {
        ...
        val targetValue = request.snapshot.restoreTarget(model)
