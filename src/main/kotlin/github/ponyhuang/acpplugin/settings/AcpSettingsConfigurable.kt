...
class AcpSettingsConfigurable : Configurable {
    ...
    private fun installRegistryAgent(
        ...
        resultHolder[0]?.let { result ->
            val isValid = when (result.installedAgent.installMethod) {
                InstallMethod.NPX, InstallMethod.UVX -> {
                    AgentInstallationUtil.validateCommand(
                        result.installedAgent.command,
                        result.installedAgent.args
                    )
                }
                InstallMethod.BINARY -> true
            }
            if (!isValid) {
                showError("Failed to validate ${result.installedAgent.installMethod} command. Make sure ${result.installedAgent.command} is installed and on your PATH.")
                return
            }
